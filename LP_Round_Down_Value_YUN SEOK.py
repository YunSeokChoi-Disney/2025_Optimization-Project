###Project2 LP Round Down Value for Knapsack Problem (2019048440 CHOI YUN SEOK)###
...
def knapsack_lp_relaxation_with_round_down(weights, values, capacity):
    ...
    # Round down the values of variables to get a feasible solution
    rounded_solution = np.floor(result.x)

    # Calculate the total value based on the rounded solution
    max_value = np.dot(values, rounded_solution)

    # The rounded solution represents the indices of items included in the solution
    Solution = [item+1 for item in range(n) if rounded_solution[item]>0]
...
if __name__ == "__main__":
    ...
    lp_relaxation_cost, lp_relaxation_solution = knapsack_lp_relaxation_with_round_down(weights, values, capacity)  
